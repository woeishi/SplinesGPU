//@author: woei
//@help: creates a catmull-rom spline ribbon along 3d coordinats, calcualted on the GPU
//@tags: curve, spline, cubic, catmull-rom
// PARAMETERS-------------------------------------------------------------------
int Size;
...
pota CatmullRomSpline(float4 p1, float4 p2, float4 p3, float4 p4, float range) {
	pota Out = (pota)0;

    float mu = frac(range);
   	float4 a0 = p4*0.5 - p3*1.5 + p2*1.5 - p1*0.5;
   	float4 a1 = -p4*0.5 + p3*2 - p2*2.5 + p1;
   	float4 a2 = p3*0.5 - p1*0.5;
	
	Out.Pos = (a0*mu*mu*mu + a1*mu*mu + a2*mu + p2);	
	Out.Tang = a0*3*mu*mu + a1*2*mu + a2;
	return Out;
}
// VERTEXSHADER-----------------------------------------------------------------
float pitch;
float4 CatmullRomSplineGPU(float4 PosO: POSITION, float3 NormO: NORMAL, float4 PosCd: TEXCOORD1, out float3 Tang, out float3 BiTang)
{
	...
	pota curve = CatmullRomSpline(p1,p2,p3,p4,PosCd.x*cSize);
